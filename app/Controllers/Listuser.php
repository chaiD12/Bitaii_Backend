...
class Listuser extends BaseController
{
    ...
    public function index()
    {
		...
		if ($check) {
            ...
            return view('listuser_view', $datas);
        }
    ...
    function get_data() {
        ...
        $sql = $this->db->query("SELECT a.id_user FROM tb_user a  "); 
        $sql_count = $sql->getNumRows($sql); 
        $sql->freeResult();

        $query = "SELECT a.*,
            (SELECT x.os_platform FROM tb_install x WHERE x.id_install=a.id_install) as os_platform 
            FROM tb_user a
            WHERE ( a.id_user LIKE '%".$search."%' OR a.email LIKE '%".$search."%' 
            OR a.user_name LIKE '%".$search."%' OR a.display_name LIKE '%".$search."%' 
            OR a.type_package LIKE '%".$search."%' ) ";
