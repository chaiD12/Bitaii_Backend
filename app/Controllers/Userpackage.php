...
class Userpackage extends BaseController
{
    ...
    public function index()
    {
		...
		if ($check) {
            ...
            return view('userpackage_view', $datas);
        }
    ...
    function get_data() {
        ...
        $sql = $this->db->query("SELECT a.id_user_package FROM tb_user_package a  "); 
        $sql_count = $sql->getNumRows($sql); 
        $sql->freeResult();

        $query = "SELECT a.*, b.user_name, b.email, b.display_name, b.expired_at 
            FROM tb_user_package a, tb_user b
            WHERE a.id_user=b.id_user 
            AND ( a.id_user_package LIKE '%".$search."%' OR b.email LIKE '%".$search."%' 
            OR b.user_name LIKE '%".$search."%' OR b.display_name LIKE '%".$search."%' 
            OR a.code_package LIKE '%".$search."%' ) ";
