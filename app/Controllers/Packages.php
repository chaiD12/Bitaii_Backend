...
class Packages extends BaseController
{
    ...
    public function index()
    {
		...
		if ($check) {
            ...
            return view('packages_view', $datas);
        }
    ...
    function get_data() {
        ...
        $sql = $this->db->query("SELECT a.id_package FROM tb_package a  "); 
        $sql_count = $sql->getNumRows($sql); 
        $sql->freeResult();

        $query = "SELECT a.*
            FROM tb_package a
            WHERE ( a.id_package LIKE '%".$search."%' OR a.code_package LIKE '%".$search."%' 
            OR a.title LIKE '%".$search."%' OR a.description LIKE '%".$search."%' 
            OR a.price LIKE '%".$search."%' OR a.currency LIKE '%".$search."%' ) ";
