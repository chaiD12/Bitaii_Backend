...
class Methodpay extends BaseController
{
    ...
    public function index()
    {
		...
		if ($check) {
            ...
            return view('methodpay_view', $datas);
        }
    ...
    function get_data() {
        ...
        $sql = $this->db->query("SELECT a.id_payment_method FROM tb_payment_method a  "); 
        $sql_count = $sql->getNumRows($sql); 
        $sql->freeResult();

        $query = "SELECT a.*
            FROM tb_payment_method a
            WHERE ( a.id_payment_method LIKE '%".$search."%' OR a.code_method LIKE '%".$search."%' 
            OR a.os_platform LIKE '%".$search."%' OR a.app_mode LIKE '%".$search."%' 
            OR a.param_key LIKE '%".$search."%' OR a.param_string LIKE '%".$search."%' ) ";
